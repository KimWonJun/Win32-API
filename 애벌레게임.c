...
{ 
	...
	case WM_CREATE:	//윈도우가 만들어졌을때
		...
		SetTimer(hwnd, 1, Speed, NULL);	//0.05초마다 움직임
		break;
	...
	case WM_PAINT:
		...
		oldPen = (HPEN)SelectObject(memdc, hPen);
		Ellipse(memdc, coordinates[0][0] - 10, coordinates[0][1] - 10, coordinates[0][0] + 10, coordinates[0][1] + 10);	//머리
		...
		TextOut(memdc, 0, 0, str, _tcslen(str));
