...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:	//윈도우가 만들어졌을때
		break;
	case WM_PAINT:	//윈도우가 그려질때
		hdc = BeginPaint(hwnd, &ps);	// == GetDC(hwnd);
		TextOut(hdc, 940, 500, _T("Hello 안녕"), _tcslen(_T("Hello 안녕")));
		rect.left = 0;
		rect.top = 0;
		rect.right = 1000;
		rect.bottom = 500;
		DrawText(hdc, _T("안녕하세요 ㅎㅎ"), _tcslen(_T("안녕하세요 ㅎㅎ")), &rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
